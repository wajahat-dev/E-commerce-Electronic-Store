...
?>
	<div style="padding-right:20px;">
		...
		<div id="bill-wrap-install" class="bill-wrap-install" style="display:none">
			<h3><?php esc_attr("Please wait","wptools");?></h3>
			<big>
				<h4>
				<?php esc_attr("Installing plugin","wptools");?> <div id="billpluginslug">...</div>
				</h4>
			</big>
			...
			<tbody class="wptools-more-plugins-body">
				...
				for ($i = 0; $i < $total; $i++) {
					...
					if (wptools_plugin_installed($plugins_to_install[$i]["slug"]))
						echo '<a href="#" class="button activate-now">'. esc_attr_e('Installed', 'wptools').'</a>';
					else
						echo '<a href="#" id="' . esc_attr($plugins_to_install[$i]["slug"]) . '"class="button button-primary wt-bill-install-now">'. esc_attr_e('Install', 'wptools').'</a>';
					echo '</td>';
					if ($counter % 2 == 1) {
...
?>
	...
	if (count($marray) < 1) {
		echo '<h3>';
		echo esc_attr__('No Javascript errors found last 3000 entries of log_error file.', 'wptools');
		echo '</h3>';
		return;
	}
	echo '<br>';
...
?>
	...
	if ($wptools_server_performance == 'yes') {
		echo '<div style="float:left">';
		echo wptools_print_html_result($benchmarkIndustryResult, $showServerName, esc_attr_e("Industry Average Data","wptools"));
		echo '</div>';
	}
}
...
function wptools_options()
{
	...
	echo '<center>';
	echo '<h2>';
	echo esc_attr__("The lasts 200 lines of the log files.", "wptools");
	echo '</h2>';
	echo '</center>';
	echo '<h4>';
...
function bill_check_resources($par)
{
	...
	?>
		<div id="wptools-steps3">
			...
			try {
				if (!@is_readable('/proc/cpuinfo') or  !@file_get_contents('/proc/cpuinfo')    ) {
					esc_attr_e("Plugin requirement:","wptools");?>
					&nbsp; 
